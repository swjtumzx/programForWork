 ...
 * ### 使用方式
 * ```js
 * import { Select } from 'mdesign-swjtumzx'
 * const { Option } = Select
 * ```
 */
export const Select: FC<SelectProps> = (props) => {
  ...
  useEffect(() => {
    if (children?.toString() !== currentActiveValue) {
      React.Children.map(children, (child) => {
        const childElement = child as FunctionComponentElement<SelectItemProps>
        if (currentActiveValue.toString() === childElement.props.value.toString()) {
          setShowText(childElement.props.children as string)
        }
      });
    }
  ...
  const selCls = classNames('m-select', className, {
  ...
  const renderChildren = () => {
    const childrenComponent = Children.map(children, (child) => {
      ...
      if (displayName === 'SelectItem') {
        const isRenderItem = filterOption && childElement && filterOption(String(currentActiveValue), childElement.props)
        if (filterOption) {
          if (isRenderItem) {
    ...
    return (
      ...
      >
        <div className="m-select-items" style={{maxHeight: `${listHeight}px`, overflow: 'auto'}} >
  ...
  const renderSearchSelector = () => (
    <div className="m-select-selection-search">
      <input
        type="text"
        ref={selectInp}
        autoFocus={autoFocus}
        ...
        className="m-select-selection-search-input" />
    </div>
  )
